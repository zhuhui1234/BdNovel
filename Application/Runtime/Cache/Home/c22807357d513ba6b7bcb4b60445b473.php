...
<style>
	...
	.file-box{ position:relative;margin-top:3px;width:180px} 
	.file{cursor:pointer;position:absolute;top:0;right:110px;height:32px;filter:alpha(opacity:0);opacity: 0;width:60px;}
</style>
	...
	<div class="row clearfix">
		<div class="col-md-2 list fl">
			...
			<h2 style="background-color:#34495E;border:1px solid #34495E;border-top:none;">
				<a href="<?php echo U('Person/inf');?>"  style="font-size:18px;height:60px;line-height:60px;color:white;">个人信息</a>
			</h2>
			<h2 style="background-color:#f2f2f0;border:1px solid #ccc;border-top:none;">
		...
		<div class="book fr" style="height:auto;">
			<span style="width:97%;height:40px;display:block;" id="tablelist">
				<form method="post" action="<?php echo U('Person/doChangeInfo');?>">
			    	...
			    	<table class="table">
			    		...
			    		<tr>
			    			<td>用户名：</td>
			    			<td>
			    				<input id="inf" type="text" name="readername" value="<?php echo ($inf['readername']); ?>">
			    			</td>
			    		</tr>
			    		<tr>
			    			<td>性别：</td>
			    			<td>
			    				<input type="radio" name="sex" value="0" <?php if($inf["sex"] == '0'): ?>checked<?php endif; ?> >女
			    				<input type="radio" name="sex" value="1" <?php if($inf["sex"] == '1'): ?>checked<?php endif; ?> >男
			    				<input type="radio" name="sex" value="2" <?php if($inf["sex"] == '2'): ?>checked<?php endif; ?> >保密
			    			</td>
			    		</tr>
			    		<tr>
			    			<td>联系电话：</td>
			    			<td>
			    				<input id="inf" type="text" name="phone" value="<?php echo ($inf['phone']); ?>">
			    			</td>
			    		</tr>
			    		<tr>
			    			<td>Email：</td>
			    			<td>
			    				<input id="inf" type="text" name="email" value="<?php echo ($inf['email']); ?>">
			    			</td>
			    		</tr>
			    		<tr>
			    			<td>自我描述：</td>
			    			<td>
			    				<textarea style="text-indent: 2em;" name="descr" cols="30" rows="4"><?php if($inf["descr"] == ''): ?>这个家伙很懒，什么都没有留下~<?php else: echo ($inf["descr"]); endif; ?></textarea>
			    			</td>
			    		</tr>
			    		<tr>
			    			<td colspan="2">
		    	...
		    	<div id="pic">
		    		<p style="font-size:20px;">头像设置</p>

		    		<?php if($inf['pic'] == ''): ?><img src="/BdNovel/Upload/reader/touxiang.jpg" alt="" width="180" height="180"><?php else: ?><img src="/BdNovel/Upload/reader/<?php echo ($inf[pic]); ?>" alt="" width="180" height="180"><?php endif; ?>
					<div class="file-box"> 
						<form action="<?php echo U('Person/changpic');?>" method="post" enctype="multipart/form-data">
							<input type="hidden" name="id" value="<?php echo ($inf['id']); ?>">
							<input type="hidden" name="oldpic" value="<?php echo ($inf['pic']); ?>">
							<input type="file" name="pic" class="file"> 
							<input type='button' class="btn btn-primary" value="　浏览..." />　
							<input type="submit"  class="btn btn-success" value="上传头像" /> 
						</form> 
					</div> 
		    	</div>
			</span>
